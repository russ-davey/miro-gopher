...
// GetAll Retrieves a list of items within a specific frame. A frame is a parent item and all items within a frame are child items.
...
// Search query params: FrameSearchParams{}
func (f *FramesService) GetAll(boardID string, queryParams ...FrameSearchParams) (*ListFrames, error) {
	response := &ListFrames{}
	...
	if url, err := constructURL(f.client.BaseURL, f.apiVersion, f.resource, boardID, f.subResource); err != nil {
		...
		if len(queryParams) > 0 {
			err = f.client.Get(f.client.ctx, url, response, ParseQueryTags(queryParams[0])...)
		} else {
			err = f.client.Get(f.client.ctx, url, response)
		}
